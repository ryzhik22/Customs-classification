...
class HTZVectorizer():
    ...
    def fit_transform(self, raw_documents, y=None):
        ...
        X_cntvec_idf = X_cntvec.toarray()
        X_cntvec_idf = scipy.sparse.csr_matrix(np.where(X_cntvec_idf > 1, 1, X_cntvec_idf))
        
        idf_matrix = np.zeros((len(self.classes), X_cntvec.shape[1]))
        for i in tqdm_notebook(range(X_cntvec.shape[0])):
            idf_matrix[y[i]] += X_cntvec_idf[i]
        ...
        for i in tqdm_notebook(range(self.htz_matrix.shape[0])):
            without_one_class = X_cntvec.shape[0]-values_class[i]
            for j in range(self.htz_matrix.shape[1]):
                ...
                freq_all_doc = np.sum(idf_matrix[:,j]) - idf_matrix[i][j] + 1e-10
                
                
         #       print(math.log(without_one_class/freq_all_doc, 2))
         #       print(IDF_wC)
                      
            #    self.htz_matrix[i][j] = (math.log(without_one_class/freq_all_doc, without_one_class)- IDF_wC)                   
           ...
           #     if self.htz_matrix[i][j] < 0:
            #        self.htz_matrix[i][j] = 0
             #   if self.htz_matrix[i][j] > 1:
              #      print(self.htz_matrix[i][j])
               #     print(math.log(without_one_class/freq_all_doc, 2))
                #    print(IDF_wC)
                    
    #    print(self.htz_matrix[0])
        arr = self.htz_matrix > 1
        if True in arr:
            print(True)
   ...
   #     X_cntvec.data = np.log(X_cntvec.data + 1)
    ...
    #    tf_matrix = np.log(X_cntvec.data + 1, docs_len)
    ...
    #    diag_lens = diags(1/np.log(docs_len + 1e-10))
      ...
      #  X_cntvec.data = np.log(X_cntvec.data + 1)
        ...
        RIC_matrix = tf_matrix @ self.htz_matrix.T
        for i in tqdm_notebook(range(RIC_matrix.shape[0])):
            RIC_matrix[i] = tf_matrix[i] @ self.htz_matrix.T
            for j in range(RIC_matrix.shape[1]):
                if np.sum(RIC_matrix[i]) - RIC_matrix[i][j] != 0:
                    RIC_matrix[i][j] = RIC_matrix[i][j]/(np.sum(RIC_matrix[i]) - RIC_matrix[i][j])
